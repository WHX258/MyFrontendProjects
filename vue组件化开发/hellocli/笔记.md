...
```
hellocli/
├── node_modules/        # 依赖包目录，如果没有该目录可以通过npm install命令安装依赖包
├── public/              # 静态资源目录
├── src/                 # 源代码目录
│   ├── assets/         # 静态资源
│   ├── components/     # 组件目录
...
例如：在`App.vue`中向`Movie`组件传递一个`movies`属性：
```vue
<template>
...
```
这就需要在`./components/Movie.vue`文件中定义包含`"id","title","rating"`字段的`props`
```vue
<template>
  <div class="movie">
    <h1>{{ title }}</h1>
...
<script>
// 导出组件选项对象，对应了app.vue中的导入
export default {
  name: 'Movie', // movie组件的名称
  props: [ // 组件之间传值使用props，可以从父组件传值到子组件
    "id","title","rating"
  ],
  data() {return {}}, // 组件自己的数据
  methods: {
    // 组件的方法，和vue基础操作是一样的
    fun(){ alert("你收藏了"+this.title) }
  }
}
</script>
```