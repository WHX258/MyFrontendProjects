...
## 3. 分支重置
在插件中，右键分支有一个 重置分支到这里 的选项，可以将当前分支重置到指定的提交。  
选项注释：  
Soft：仅移动HEAD指针，所有文件内容没变 
Mixed：移动HEAD指针，并更新暂存区以匹配指定版本，但保留工作目录中的更改。  
Hard：回滚到指定版本，并删除指定版本之后的所有更改。  
Keep：回滚到指定版本，但是保留指定版本之后的更改。
...
c. 当删除分支时，引用和日志会被删除，但commit对象和blob对象仍然存在于.git/objects中，直到垃圾回收（git gc）运行时才会被清理。
